...
function applyAtividadesFilters(query: any, filters?: AtividadeFilters) {
  let q = query as any;
  if (filters?.tipos?.length) q = q.in('tipo', filters.tipos);
  ...
  // Filtrar por período: atividades cuja data_inicio está dentro do intervalo
  if (filters?.data_inicio) q = q.gte('data_inicio', filters.data_inicio);
