...
/** Gera a Tabela de Vendas em PDF (apenas unidades disponíveis). */
export async function exportUnidadesPdf({
  ...
  const unidadesFonte = unidades.filter((u) => u.status === 'disponivel');
  ...
  const reportTitle = 'Tabela de vendas';
  ...
  const head: any = showFluxo
    ...
    : [headBase([{ content: 'Valor total', rowSpan: 1 }]).map((c) => ({ content: c.content }))];
  ...
  const drawHeader = () => {
    ...
    const refLabel = plano.ref_label ? `  ·  Ref: ${plano.ref_label}` : '';
  ...
  let y = (tableDoc.lastAutoTable?.finalY ?? 30) + 8;
  ...
  for (let page = 1; page <= totalPages; page += 1) {
    ...
    doc.text('Disponibilidade sujeita a alteração sem aviso prévio', marginX, pageHeight - 6);
  ...
  if (download) {
    doc.save(`Tabela_Vendas_${safeName}_${dateStamp}.pdf`);
