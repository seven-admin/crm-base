...
const menuGroupsDef = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: BarChart2, label: 'Admin & Leads', path: '/arqo/admin', moduleName: 'arqo_admin' },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: FileSignature, label: 'Modelos', path: '/nexa/contratos/modelos', moduleName: 'nexa_contratos_modelos' },
      { icon: BookOpen, label: 'Blocos de Texto', path: '/nexa/contratos/blocos', moduleName: 'nexa_contratos_blocos' },
      { icon: Variable, label: 'Variáveis', path: '/nexa/contratos/variaveis', moduleName: 'nexa_contratos_variaveis' },
    ],
  },
  {
    label: 'Clientes',
...
export function Sidebar() {
  ...
  const renderMenuItem = (item: MenuItem, showLabel: boolean, groupColor?: string) => {
    ...
    return (
      ...
      >
        <item.icon className="h-4 w-4 flex-shrink-0" style={groupColor ? { color: groupColor } : undefined} />
    ...
    return (
      ...
      >
        <CollapsibleTrigger asChild>
          ...
          >
            <div className="flex items-center gap-3">
              {GroupIcon && <GroupIcon className="h-4 w-4 flex-shrink-0" style={{ color: group.color }} />}
        ...
        <CollapsibleContent className="pl-4 space-y-0.5 mt-1 overflow-hidden data-[state=open]:animate-collapsible-down data-[state=closed]:animate-collapsible-up">
          {group.items.map((item) => renderMenuItem(item, true, group.color))}
        </CollapsibleContent>
      </Collapsible>
    );
  };
