...
async function cleanupReferences(admin: ReturnType<typeof createClient>, userId: string) {
  const steps: Array<{ label: string; critical?: boolean; run: () => Promise<any> }> = [
    ...
    { label: 'nexa_visitas.created_by', critical: true,
      run: () => admin.from('nexa_visitas').update({ created_by: null }).eq('created_by', userId) },
    ...
    { label: 'seven_cliente_interacoes.user_id', critical: true,
      run: () => admin.from('seven_cliente_interacoes').update({ user_id: null }).eq('user_id', userId) },
    { label: 'seven_unidade_historico_precos.alterado_por', critical: true,
      run: () => admin.from('seven_unidade_historico_precos').update({ alterado_por: null }).eq('alterado_por', userId) },
    { label: 'sistema_audit_logs.user_id', critical: true,
      run: () => admin.from('sistema_audit_logs').update({ user_id: null }).eq('user_id', userId) },
    { label: 'nexa_contratos.created_by', critical: true,
      run: () => admin.from('nexa_contratos').update({ created_by: null }).eq('created_by', userId) },
    { label: 'nexa_contrato_templates.created_by', critical: true,
