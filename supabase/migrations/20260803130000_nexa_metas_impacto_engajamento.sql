-- NEXA · metas semanais de Impacto (corretores distintos) e Engajamento (corretores
-- em 2+ atividades), conforme o rascunho do dashboard por consultor.

ALTER TABLE public.nexa_metas
  ADD COLUMN meta_semanal_impacto integer NOT NULL DEFAULT 0 CHECK (meta_semanal_impacto >= 0),
  ADD COLUMN meta_semanal_engajamento integer NOT NULL DEFAULT 0 CHECK (meta_semanal_engajamento >= 0);

DROP FUNCTION IF EXISTS public.nexa_salvar_meta(text, date, date, integer, integer, boolean, uuid[], uuid);
...
BEGIN
  ...
  IF p_meta_semanal_visitas < 0 OR p_meta_semanal_atendimentos < 0
    OR p_meta_semanal_impacto < 0 OR p_meta_semanal_engajamento < 0 THEN
  ...
  IF p_meta_id IS NULL THEN
    INSERT INTO public.nexa_metas (
      ...
      meta_semanal_impacto, meta_semanal_engajamento, is_active
    ) VALUES (
      btrim(p_nome), p_vigencia_inicio, p_vigencia_fim,
      p_meta_semanal_visitas, p_meta_semanal_atendimentos,
      p_meta_semanal_impacto, p_meta_semanal_engajamento, p_is_active
    )
